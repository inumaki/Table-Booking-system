...
{
	var baseurl="http://localhost/bookingsystem/AdminPortal/home/table/";
		jQuery.ajax({
		...
		success:function(data){
				...
				var pre_text=`<tr><th>ID</th><th>Table Number</th><th>Booking Price</th><th>Number Of Seats</th></tr>`;
				var final_text='';
				var text='';
				for(let i in  table_data)
				{
					text+=`<tr><td>`+table_data[i].table_number+`</td><td>`+table_data[i].table_cost+`</td><td>`+table_data[i].number_of_seats+`</td><td><button onclick="view_data(`+table_data[i].id+`);" class="w3-btn w3-blue">View</button>&nbsp;&nbsp;<button onclick="edit_data(`+table_data[i].id+`);" class="w3-btn w3-green">Edit</button>&nbsp;&nbsp;<button onclick="delete_data(`+table_data[i].id+`);"class="w3-btn w3-red">Delete</button></td></tr>`;
				}
